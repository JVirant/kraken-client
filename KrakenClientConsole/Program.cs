    ...
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            if (args.Length == 0)
            {
                ...
                Console.WriteLine(AppDomain.CurrentDomain.FriendlyName + " balance");
                Console.WriteLine(AppDomain.CurrentDomain.FriendlyName + " <buy/sell> <btc amount> <eur price>");
                Console.WriteLine(AppDomain.CurrentDomain.FriendlyName + " cancel <txid>");
                Console.WriteLine(AppDomain.CurrentDomain.FriendlyName + " orders");
                return;
            }
            ...
            switch(args[0])
            {
                ...
                case "sell":
                case "buy":
                    var volume = decimal.Parse(args[1]);
                    var price = decimal.Parse(args[2]);
                    Console.Write(args[0] + " " + volume + " BTC for " + price + " EUR ? (y/n) ");
                    if (Console.ReadLine() != "y")
                        break;
                    var order = client.AddOrder("XXBTZEUR", args[0], "limit", volume, price, null);
                    Console.WriteLine("add order result: " + order.ToString());
                    break;

                case "cancel":
                    var cancelOrder = client.CancelOrder(args[1]);
